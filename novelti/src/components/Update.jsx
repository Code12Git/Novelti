...
const Update = ({ data }) => {
  const [user, setUser] = useState({
    ...
    selectedCountry: {
      value: data.selectedCountry[0].label,
      label: data.selectedCountry[0].label,
    },
    selectedStates: data.selectedStates.map((state) => ({
      value: state,
      label: state,
    })),
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(`/users/${data._id}`, userData);
